...
GLMotif::PopupMenu *Sandbox::createMainMenu(void) {
    ...
    if (waterTable != 0) {
        ...
        GLMotif::Button *showWaterControlDialogButton = new GLMotif::Button("ShowWaterControlDialogButton", mainMenu,
...
GLMotif::PopupWindow *Sandbox::createWaterControlDialog(void) {
    ...
    GLMotif::RowColumn *waterControlDialog = new GLMotif::RowColumn("WaterControlDialog", waterControlDialogPopup,
                                                                    false);
    ...
    waterAttenuationSlider = new GLMotif::TextFieldSlider("WaterAttenuationSlider", waterControlDialog, 8,
                                                          ss.fontHeight * 10.0f);
...
//    Math::Interval<double> elevationRange = cfg.retrieveValue<Math::Interval<double> >("./elevationRange",
//                                                                                       Math::Interval<double>(-1000.0,
//                                                                                                              1000.0));
...
//    Math::Interval<double> rainElevationRange = cfg.retrieveValue<Math::Interval<double> >("./rainElevationRange",
...
//                    Misc::ValueCoder<Geometry::Point<double, 3> >::decode(s.c_str(), s.c_str() + s.length()));
//        }
//    }
//
...
//    frameFilter = new FrameFilter(frameSize, numAveragingSlots, pixelDepthCorrection, cameraIps.depthProjection,
//                                  basePlane);
...
//        ONTransform::Vector x =
//                (basePlaneCorners[1] - basePlaneCorners[0]) + (basePlaneCorners[3] - basePlaneCorners[2]);
                ...
                if (isToken(tokens[0], "waterSpeed")) {
                    ...
                    if (tokens.size() == 2) {
                        try {
                            ...
                            std::cerr << "Cannot read height color map " << tokens[1] << " due to exception "
                                      << err.what() << std::endl;
                        }
                    } else
                    ...
                    if (tokens.size() == 2) {
                        ...
                        if (isToken(tokens[1], "on") || isToken(tokens[1], "off")) {
                            ...
                            std::cerr << "Invalid parameter " << tokens[1]
                                      << " for useContourLines control pipe command" << std::endl;
                    } else
                        std::cerr << "Wrong number of arguments for contourLineSpacing control pipe command"
                                  << std::endl;
...
void Sandbox::display(GLContextData &contextData) const {
    ...
    const RenderSettings &rs =
            windowIndex < int(renderSettings.size()) ? renderSettings[windowIndex] : renderSettings.back();
